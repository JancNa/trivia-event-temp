...
export default function LeaderboardView() {
  ...
  const loadData = async (showRefreshIndicator = false, boardId = viewLeaderboardId) => {
    if (!boardId) return;
  ...
  const metrics = useMemo(() => {
    ...
    const totalXP = leaderboard.reduce((sum, item) => sum + Number(item.total_xp), 0);
    return {
      totalPlayers,
      totalXP,
    };
  ...
  const spawnFakePlayer = async () => {
    ...
    if (!isDemoMode()) {
      alert('Activa el "Modo Demo" en el botón de configuración de arriba para poder simular competidores sin conexión.');
      return;
    }
    const names = [
      'Catalina Gómez', 'Mateo Rivera', 'Paula Torres', 'Lucas Silva', 
      'Valentina Vega', 'Andrés Martínez', 'Alejandra Muñoz', 'Felipe Ortiz'
    ];
    const pickedName = names[Math.floor(Math.random() * names.length)] + ` (Sim # ${Math.floor(Math.random()*100)})`;
    await insertMockCompetitor(pickedName, Math.random() * 0.4 + 0.6, viewLeaderboardId);
  ...
  if (!viewLeaderboardId) {
    return (
      <div 
        ...
        style={{
          backgroundImage: "linear-gradient(rgba(17, 18, 17, 0.72), rgba(17, 18, 17, 0.82)), url('https://images.unsplash.com/photo-1527018601619-a508a2be00cd?q=80&w=1600')",
      ...
      >
        ...
        <div className="max-w-md w-full mx-auto mt-20 mb-12 relative z-10">
          <div className="bg-white/98 text-slate-800 rounded-[32px] shadow-[0_25px_60px_-15px_rgba(0,0,0,0.6)] border border-white/20 overflow-hidden p-6 relative">
            ...
            <p className="text-xs text-slate-500 mb-6 font-medium leading-relaxed">
              Selecciona uno de los rankings o competiciones activas para desplegar la transmisión en tiempo real para todos los espectadores.
            </p>

            {loadingBoards ? (
              <div className="py-12 flex flex-col items-center justify-center gap-3">
                <RefreshCw className="w-8 h-8 text-[#fed600] animate-spin" />
                <span className="text-xs font-mono text-slate-400">Cargando eventos...</span>
              </div>
                ...
                {leaderboardsList.map((board) => {
                  ...
                  return (
                    <button
                      key={board.id}
                      onClick={() => {
                        localStorage.setItem('view_selected_leaderboard_id', board.id);
                    ...
                    >
                      <div className="flex-1 min-w-0 pr-2">
                        ...
                        <h4 className="text-xs sm:text-sm font-bold text-slate-800 truncate group-hover:text-[#e52b12] transition-colors">
                          {board.name}
                        </h4>
                        <div className="flex items-center gap-1.5 mt-2">
                          ...
                          <span className={`text-[10px] font-mono font-extrabold uppercase ${
                            isResults ? 'text-red-400' : isRegistration ? 'text-amber-400' : 'text-green-500'
                          }`}>
                            {isResults ? 'Finalizado' : isRegistration ? 'Borrador' : 'Activo'}
                          </span>
                        </div>
                      </div>
                      <ChevronRight className="w-4 h-4 text-slate-400 group-hover:text-[#e52b12] transition-colors shrink-0" />
                    </button>
                  );
                })}
              </div>
            ...
            <div className="mt-6 pt-5 border-t border-slate-100 flex justify-between items-center text-xs">
              ...
              <Link to="/admin" className="text-[#e52b12] font-black hover:underline select-none">
                Ir a Admin →
              </Link>
            </div>
          </div>
        ...
        <footer className="mt-8 text-center text-[10px] text-white/35 font-semibold uppercase tracking-widest leading-none select-none py-4 border-t border-white/5">
          Dinámica de Trivia Realtime • Panel de Difusión Central • Corporativo Leal 360
        </footer>
      </div>
    );
  }
    ...
    >
      ...
      <div className="absolute top-4 left-4 right-4 z-50 flex items-center justify-between pointer-events-none">
        ...
        <div className="flex items-center gap-2 pointer-events-auto">
          ...
          >
            <RefreshCw className={`w-4 h-4 ${isRefreshing ? 'animate-spin text-[#fed600]' : ''}`} />
          </button>
          ...
          <button
            ...
            className="p-2 bg-red-600/30 hover:bg-red-600/55 border border-red-500/30 text-red-200 hover:text-white rounded-xl transition-all cursor-pointer backdrop-blur shadow-lg"
            title="Bloquear Panel"
            id="btn-leaderboard-logout"
          >
      ...
      <div className="max-w-4xl w-full mx-auto mt-20 mb-12 relative z-10 transition-all duration-300">
        
        {/* LEAL FLOATING LOGO */}
        <div className="absolute -top-16 left-1/2 -translate-x-1/2 z-30 transition-transform duration-300 hover:scale-105">
          <div className="w-32 h-32 select-none">
            <svg viewBox="0 0 100 100" className="w-full h-full filter drop-shadow-[0_4px_10px_rgba(0,0,0,0.3)]">
        ...
        {/* CORE WHITE CARD */}
        <div className="bg-white/98 text-slate-800 rounded-[32px] shadow-[0_25px_60px_-15px_rgba(0,0,0,0.6)] border border-white/20 overflow-hidden relative" id="leaderboard_primary_board">
          ...
          <div className="grid grid-cols-3 divide-x divide-slate-100 bg-[#fafafa] border-b border-slate-100 pt-20 pb-6 px-4 text-center">
            <div className="flex flex-col justify-center items-center">
              ...
              <span className="text-2xl sm:text-3.5xl font-black text-[#111211] tracking-tight mt-1" id="metric-participants">
            ...
            <div className="flex flex-col justify-center items-center">
              ...
              <span className="text-2xl sm:text-3.5xl font-black text-[#111211] tracking-tight mt-1" id="metric-total-xp">
            ...
            <div className="flex flex-col justify-center items-center">
              ...
              <span className="text-2xl sm:text-3.5xl font-black text-[#e52b12] tracking-tight mt-1">
                TOP 30
              </span>
            </div>
          </div>
          ...
          <div className="border-b border-slate-100">
            ...
            >
              <Gift className="w-4 h-4 text-[#e52b12] shrink-0" />
              <span>Premios Disponibles del Evento</span>
              {showPrizes ? <ChevronUp className="w-4 h-4 text-slate-400" /> : <ChevronDown className="w-4 h-4 text-slate-400" />}
            </button>
            ...
            <AnimatePresence>
              {showPrizes && (
                ...
                >
                  <div className="p-6 border-t border-slate-100 grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs font-medium">
                    <div className="p-3.5 bg-yellow-50/60 border border-yellow-100 rounded-2xl flex gap-3.5 items-center">
                      <div className="text-2xl shrink-0">👑</div>
                      <div>
                        <div className="font-bold text-[#111211]">1er Lugar</div>
                        <p className="text-[10px] text-slate-500 mt-0.5">200.000 Puntos Leal + Trofeo Especial</p>
                      </div>
                    </div>
                    ...
                    <div className="p-3.5 bg-slate-50 border border-slate-200/60 rounded-2xl flex gap-3.5 items-center">
                      <div className="text-2xl shrink-0">🥈</div>
                      <div>
                        <div className="font-bold text-[#111211]">2do Lugar</div>
                        <p className="text-[10px] text-slate-500 mt-0.5">100.000 Puntos Leal</p>
                      </div>
                    </div>
                    ...
                    <div className="p-3.5 bg-amber-50/50 border border-amber-100 rounded-2xl flex gap-3.5 items-center">
                      <div className="text-2xl shrink-0">🥉</div>
                      <div>
                        <div className="font-bold text-[#111211]">3er Lugar</div>
                        <p className="text-[10px] text-slate-500 mt-0.5">50.000 Puntos Leal</p>
                      </div>
                    </div>
                  </div>
                </motion.div>
          ...
          <div className="p-6 md:p-8 space-y-8">
            ...
            {leaderboard.length === 0 ? (
              <div className="py-12 text-center text-slate-400 border-2 border-dashed border-slate-100 rounded-3xl" id="podium_empty">
                <HelpCircle className="w-12 h-12 text-slate-200 mx-auto mb-3" />
                <p className="text-sm font-semibold">Esperando a que los participantes respondan para armar el podio...</p>
                <p className="text-xs mt-1 text-slate-400">Las respuestas de los jugadores se verán reflejadas aquí en tiempo real.</p>
              </div>
            ) : (
              <div className="pt-6 relative max-w-2xl mx-auto flex justify-center items-end gap-3 sm:gap-6 min-h-[290px]" id="podium_wrap">
                  ...
                  {podium.second ? (
                    ...
                    >
                      <div className="w-11 h-11 bg-slate-100 border-2 border-slate-300 rounded-full flex items-center justify-center text-lg shadow-sm relative z-20">
                        🥈
                        <span className="absolute -bottom-1 -right-1 w-5 h-5 bg-[#111211] text-white text-[10px] font-black rounded-full flex items-center justify-center">2</span>
                      </div>
                      <div className="mt-2 font-display text-xs sm:text-sm font-black text-slate-800 truncate max-w-[90px] uppercase">
                        {podium.second.name.split(' ')[0]}
                      </div>
                      <div className="text-xs font-black text-slate-500 font-mono mt-0.5">
                        {Number(podium.second.total_xp).toLocaleString()} <span className="text-[10px] opacity-70">XP</span>
                      </div>
                    </motion.div>
                  ) : (
                    <span className="text-[10px] text-slate-300 font-bold mb-2">#2 Vacío</span>
                  )}
                  {/* Visual Solid Block in Charcoal Black */}
                  <motion.div 
                    initial={{ height: 0 }}
                    animate={{ height: 95 }}
                    className="w-full bg-[#1c1e1c] rounded-t-2xl shadow-md mt-4 relative overflow-hidden"
                  >
                ...
                <div className="w-28 sm:w-36 flex flex-col items-center">
                  {podium.first ? (
                    ...
                    >
                      {/* Top crown badge */}
                      <div className="w-14 h-14 bg-red-100 border-2 border-red-500 rounded-full flex items-center justify-center text-2xl shadow-md relative z-20">
                        👑
                        <span className="absolute -bottom-1 -right-1 w-6 h-6 bg-[#e52b12] text-white text-xs font-black rounded-full flex items-center justify-center ring-2 ring-white">1</span>
                      </div>
                      <div className="mt-2.5 font-display text-sm sm:text-base font-black text-[#111211] truncate max-w-[110px] uppercase tracking-wide">
                        {podium.first.name.split(' ')[0]}
                      </div>
                      <div className="text-sm font-black text-[#e52b12] font-mono mt-0.5">
                        {Number(podium.first.total_xp).toLocaleString()} <span className="text-[11px] font-bold">XP</span>
                      </div>
                    </motion.div>
                  ) : (
                    <span className="text-[10px] text-slate-300 font-bold mb-2">#1 Vacío</span>
                  )}
                  {/* Visual Solid Block in Scarlet Red */}
                  <motion.div 
                    initial={{ height: 0 }}
                    animate={{ height: 135 }}
                    className="w-full bg-[#e52b12] rounded-t-2xl shadow-[0_4px_25px_rgba(229,43,18,0.35)] mt-4 relative overflow-hidden"
                  >
                ...
                <div className="w-24 sm:w-32 flex flex-col items-center">
                  {podium.third ? (
                    ...
                    >
                      <div className="w-11 h-11 bg-slate-100 border-2 border-slate-300 rounded-full flex items-center justify-center text-lg shadow-sm relative z-20">
                        🥉
                        <span className="absolute -bottom-1 -right-1 w-5 h-5 bg-slate-500 text-white text-[10px] font-black rounded-full flex items-center justify-center">3</span>
                      </div>
                      <div className="mt-2 font-display text-xs sm:text-sm font-black text-slate-800 truncate max-w-[90px] uppercase">
                        {podium.third.name.split(' ')[0]}
                      </div>
                      <div className="text-xs font-black text-slate-500 font-mono mt-0.5">
                        {Number(podium.third.total_xp).toLocaleString()} <span className="text-[10px] opacity-70">XP</span>
                      </div>
                    </motion.div>
                  ) : (
                    <span className="text-[10px] text-slate-300 font-bold mb-2">#3 Vacío</span>
                  )}
            ...
            <div className="relative pt-4 max-w-2xl mx-auto w-full">
              ...
              <input
                ...
                placeholder="Buscar leaderboards..."
                className="w-full pl-11 pr-4 py-3 bg-[#f5f5f7] hover:bg-[#eaeaea]/60 focus:bg-white border-2 border-slate-250/20 focus:border-[#fed600] text-slate-900 rounded-2xl text-xs sm:text-sm font-semibold transition-all focus:outline-none placeholder-slate-400 shadow-sm"
                id="search_leaderboards"
              />
            </div>

            {/* TABLE LISTING */}
            <div className="bg-[#fcfbfb]/50 border border-slate-100 rounded-3xl overflow-hidden mt-6" id="leaderboard-full-ranking">
              <div className="divide-y divide-slate-100">
                {filteredLeaderboard.length === 0 ? (
                  ...
                  filteredLeaderboard.map((row) => {
                    ...
                    const progressPercent = Math.min(100, Math.max(10, (Number(row.total_xp) / maxScore) * 100));
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3.5 min-w-0 flex-1">
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${bgCircle} text-xs font-mono`}>
                            <span className={numBadgeClass}>{row.rank}</span>
                          </div>

                          <div className="min-w-0 flex-1">
                            <div className="font-bold text-slate-800 text-xs sm:text-sm uppercase tracking-wide truncate">
                              {row.name}
                            </div>
                          </div>

                          {/* MIDDLE PROPORTIONAL PROGRESS BAR (Red line / soft outline background) */}
                        ...
                        <div className="flex items-center gap-4 shrink-0 justify-end">
                          ...
                          {/* OUTLINE TROPHY BADGE (Coral styled outliner) */}
                          <div className="px-2.5 py-1.5 bg-[#e52b12]/5 border border-[#e52b12]/15 text-[#e52b12] text-[10px] font-extrabold uppercase rounded-lg tracking-widest flex items-center gap-1 shadow-sm select-none">
        ...
        <div className="mt-4 text-center">
          <p className="text-[10px] sm:text-xs font-extrabold text-white/50 tracking-wide">
            Actualizado en tiempo real • {new Date().toLocaleDateString('es-ES', { day: 'numeric', month: 'long', year: 'numeric' })}
          </p>
        </div>
